...
class endecoder2(nn.Module):
    def __init__(self, d_model, heads, dropout, activation, flag):
        ...
        self.multihead_attn1 = nn.MultiheadAttention(d_model, heads)
        ...
        self.multihead_attn2 = nn.MultiheadAttention(d_model, heads)
...
class interactive(nn.Module):
    ...
    def forward(self, rgb, depth): #[16, 16, 64, 64]
        n, c, h, w = rgb.size()
        rgb = self.conv1(rgb)
        ...
        rgb1, depth1 = x # [16, 4096, 16]
        res = rgb1.permute(0, 2, 1)
        res1 = depth1.permute(0, 2, 1)
        output = res.reshape(n, c, h, w)
        output1 = res1.reshape(n, c, h, w)
